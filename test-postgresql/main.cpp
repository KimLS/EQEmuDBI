...
int main() {
	try {
		...
		auto dbh = new DBI::PGDatabaseHandle();
		dbh->Connect("eqdb", "eqdb.cklzulhbla8r.us-east-1.rds.amazonaws.com", "eqdb", "eqdbpass", attr);
		...
		auto sth = dbh->Prepare("INSERT INTO db_test (id, int_value, real_value, text_value, blob_value) VALUES(?, ?, ?, ?, ?)", "InsertDbTest");
		if(!sth) {
			PrintErr("Failed to create prepared statement.");
			return 1;
		}
		
		sth->Execute(6, 2134, 125.9, std::string("Test value"), blob_value);

		sth = dbh->Prepare("SELECT int_value, real_value, text_value, blob_value FROM db_test WHERE id = ?", "SelectDbTest");
		auto rs = sth->Execute(1);
